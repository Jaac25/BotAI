...
export default function Home() {
  return (
    <div className="w-screen h-screen flex">
      <main className="flex w-full h-full flex-col bg-green-100 justify-center items-center relative">
        <div className="absolute inset-0 grid grid-cols-8 gap-8 opacity-10 pointer-events-none">
          {Array.from({ length: 80 }).map((_, i) => (
            <div key={i}>{i % 2 === 0 ? robot1 : robot2}</div>
          ))}
        </div>
        <div className="p-4 relative flex z-10 flex-col md:w-1/2 w-11/12 max-h-11/12 h-11/12 shadow rounded bg-white gap-2 shadow-gray-400">
          <h1 className="text-center text-xl font-bold">
            ¿Necesitas información de algún producto?
          </h1>
          <Chat />
        </div>
      </main>
